...
import { useAddFavourite, useGetFavourite, useDeleteFavourite } from "@/hooks/contracts/useContractFavourite";
...
const Product = ({profile, id, setError, setLoading, clear, option }: any) => {
  // Variable to store favourite
  ...
  // Use the useAddFavourite hook to add the product with the id passed in, via the marketplace contract to favourite
  const { writeAsync: addFavourite }: any = useAddFavourite([Number(id)]);
  // Use the useDeleteFavourite hook to delete the product with the id passed in, via the marketplace contract to favourite
  const { writeAsync: deleteFavourite }: any = useDeleteFavourite([Number(id)]);

  const { data: getFavourite }: any = useGetFavourite([address, id], true);
      ...
      <p className="group">
        ...
        <div className={"m-5"}>
          <div className={"pt-1"}>
            ...
            <p className="mt-4 text-2xl font-bold flex items-center justify-between">
          ...
          <div>
            <div className={"flex items-center justify-between flex-row"}>
              ...
              <p className="text-sm text-gray-700">Options: 
