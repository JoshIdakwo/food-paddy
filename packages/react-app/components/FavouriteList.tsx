...
import { useGetFavouriteLength } from "@/hooks/contracts/useContractFavourite";
import { useAccount } from "wagmi";

// The FavouriteList component is used to add a product to the marketplace
const FavouriteList = () => {
  const { address } = useAccount()
  // Use the useContractCall hook to read how many products are in the marketplace contract
  const { data }:any = useGetFavouriteLength([address], true);
  // Convert the data to a number
  const favLength = data ? Number(data.toString()) : 0;
