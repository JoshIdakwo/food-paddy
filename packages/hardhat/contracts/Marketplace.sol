...
contract Marketplace {
    ...
    struct Product {
        ...
        uint256 sold;
        // List of options of this product, might be
        // breakfast, lunch, dinner or a combination of any
        string[] options;
    }

    // Mapping of products to their index
    mapping(uint256 => Product) public products;
    // Mapping to store all the favourites of the user
    mapping(address => mapping(uint256 => Product)) public favourites;
    // Store the length of the user's favourite list
    mapping(address => uint256) public favsLength;
    ...
    // Add a product to a user favourite list
    function addToFavourite(uint256 _index) public {
        Product memory product = products[_index];
        Product memory favourite = favourites[msg.sender][_index];
        require(favourite.owner == address(0), "Product already in favourite");
        favourites[msg.sender][_index] = product;
        favsLength[msg.sender]++;
    }

    // Delete a product from a users favourite list
    function deleteFromFavourite(uint256 _index) public {
        Product memory favourite = favourites[msg.sender][_index];
        require(favourite.owner != address(0) , "Product does not exist in favourite");

        delete favourites[msg.sender][_index];
        favsLength[msg.sender]--;
    }
}